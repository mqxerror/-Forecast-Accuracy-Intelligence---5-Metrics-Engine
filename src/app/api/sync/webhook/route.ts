 ...
 *
 * Handles:
 * 1. Full variant data import (event: "import_variants")
 * 2. Sync status updates (event: "sync_completed", "sync_failed")
 */
...
async function handleVariantImport(body: { variants?: unknown[]; sync_id?: string }, startTime: number) {
  ...
  try {
    // Extract variants from request
    let variants: Record<string, unknown>[] = []

    if (body.variants && Array.isArray(body.variants)) {
      variants = body.variants as Record<string, unknown>[]
    } else if (Array.isArray(body)) {
      // n8n might send array directly
      variants = body as unknown as Record<string, unknown>[]
    }
